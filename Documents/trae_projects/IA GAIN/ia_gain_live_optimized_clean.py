...
Versao otimizada com parametros ajustados para mercado atual
"""
...
class IA_GAIN_LiveTradingOptimized:
    """Trading ao vivo otimizado para condicoes de mercado atuais"""
    
    def __init__(self):
        self.connected = False
        self.account_info = None
        
        # Parametros OTIMIZADOS para mercado atual
        self.max_positions = 5  # Aumentado para mais oportunidades
        self.max_risk_per_trade = 0.01  # Reduzido para 1% (mais conservador)
        self.min_confidence = 0.55  # Reduzido para 55% (mais flexivel)
        self.risk_reward_ratio = 2.0  # Aumentado para 1:2 (melhor R/R)
        self.volume_base = 0.01  # Volume minimo para teste
        
        # Filtros OTIMIZADOS
        self.filters = {
            'max_spread': 50,  # Aumentado para 50 pontos (mercado volatil)
            'min_volatility': 0.0001,  # Reduzido para detectar movimentos pequenos
            'max_volatility': 0.02,  # Aumentado para tolerar volatilidade
            'volume_threshold': 1.5,  # Reduzido para 1.5x (mais sensivel)
            'trend_confirmation_periods': 2,  # Reduzido para 2 periodos
        }
        
        # Simbolos otimizados (baseado no que temos disponivel)
        self.trading_symbols = [
            'EURUSDm', 'GBPUSDm', 'USDJPYm', 'AUDUSDm', 'USDCHFm',
            'USDCADm', 'NZDUSDm', 'EURJPYm', 'GBPJPYm', 'AUDJPYm'
        ]
        
        # Horario estendido (mercado forex 24h)
        self.trading_hours = {
            'start': 0,   # 00:00
            'end': 24,    # 24:00 (24 horas)
            'avoid_news': True,  # Evitar horarios de noticias
        }
    ...
    def connect_to_mt5(self) -> bool:
        ...
        try:
            ...
            logger.info("Parametros otimizados carregados")
    ...
    def check_trading_conditions(self) -> bool:
        """Verifica condicoes gerais de trading"""
        try:
            ...
            if open_positions >= self.max_positions:
                logger.warning(f"Maximo de posicoes: {open_positions}/{self.max_positions}")
                return False
    ...
    def advanced_market_scanner(self, symbol: str) -> Dict:
        """Scanner avancado de mercado com multiplos filtros otimizados"""
        try:
    ...
    def _multi_timeframe_scanner(self, symbol: str) -> Dict:
        ...
        try:
            # Timeframes: M5, M15, H1 (mais ageis)
            timeframes = [
            ...
            for tf, tf_name, periods in timeframes:
                ...
                # Analise rapida: preco vs medias moveis
                current_price = df['close'].iloc[-1]
                sma_fast = df['close'].tail(5).mean()  # 5 periodos
                sma_slow = df['close'].tail(10).mean()  # 10 periodos
            ...
            # Consolidacao com pesos diferentes
    ...
    def _momentum_scanner(self, symbol: str) -> Dict:
        """Scanner de momento com indicadores rapidos"""
        try:
            # Dados M5 para analise de momento
            ...
            # ROC (Rate of Change) - 10 periodos
            ...
            # ROC
            if abs(current_roc) < 0.2:  # Momento estavel
                momentum_score += 1
            ...
            # Volume
            if volume_ratio > 1.5:  # Volume acima da media
                momentum_score += 1
    ...
    def _volatility_scanner(self, symbol: str) -> Dict:
        ...
        try:
            # M15 para analise de volatilidade
    ...
    def _consolidate_scanner_results(self, multi_tf: Dict, momentum: Dict, volatility: Dict) -> Dict:
        ...
        try:
            ...
            if volatility.get('signal') == 'favorable':
                ...
                reasons.append(volatility.get('reason', 'Volatilidade favoravel'))
            ...
            return {'signal': 'HOLD', 'confidence': 0.5, 'reasons': ['Erro na consolidacao']}
    
    def execute_live_trade(self, analysis: Dict) -> bool:
        """Executa trade ao vivo com precisao"""
        try:
            ...
            if signal not in ['BUY', 'SELL']:
                logger.info(f"{symbol}: Sinal HOLD, nenhuma acao necessaria")
                return False
            
            logger.info(f"Executando {signal} para {symbol} (Confianca: {confidence:.1%})")
            ...
            request = {
                ...
                "sl": sl,
                "tp": tp,
                "deviation": 15,  # Maior tolerancia
                "magic": 234001,  # Identificador unico
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                logger.error(f"Falha na ordem: {result.comment} (Codigo: {result.retcode})")
                return False
    ...
    def calculate_dynamic_sl_tp(self, symbol: str, entry_price: float, signal: str) -> Tuple[float, float]:
        """Calcula SL/TP dinamico baseado na volatilidade atual"""
        try:
            # Obter ATR recente
            ...
            if rates is not None and len(rates) > 0:
                ...
                atr = entry_price * 0.0005  # ATR padrao
    ...
    def scan_and_trade_optimized(self) -> int:
        ...
        try:
            ...
            # Embaralhar simbolos para evitar vies
            import random
            ...
            for symbol in symbols_to_scan:
                ...
                logger.info(f"{symbol}: {signal} (Confianca: {confidence:.1%})")
                
                # Executar se atender aos criterios
                ...
                # Pequena pausa entre analises
                time.sleep(0.5)
            
            logger.info(f"Scan concluido - {trades_executed} trades executados")
    ...
    def monitor_session_performance(self):
        ...
        try:
            ...
            logger.info(f"Performance da sessao:")
    ...
    def run_optimized_trading_session(self, cycles: int = 3, cycle_interval: int = 300):
        """Executa sessao de trading otimizada"""
        try:
            logger.info("INICIANDO SESSAO DE TRADING OTIMIZADA")
            logger.info("="*60)
            ...
            for cycle in range(1, cycles + 1):
                ...
                if not self.check_trading_conditions():
                    logger.warning("Condicoes nao favoraveis, pulando ciclo")
            ...
            logger.info("\nSessao interrompida pelo usuario")
        except Exception as e:
            logger.error(f"Erro durante sessao: {e}")
    
    def _display_final_summary(self, total_trades: int):
        """Exibe resumo final da sessao"""
        logger.info("\n" + "="*60)
        logger.info("RESUMO FINAL DA SESSAO OTIMIZADA")
        logger.info("="*60)
        ...
        logger.info("\nSessao de trading otimizada concluida!")
    
    def _save_session_report(self, pnl: float, total_trades: int):
        """Salva relatorio detalhado da sessao"""
        try:
            report = {
...
def main():
    ...
    print("="*60)
    print("AVISO: Este script executara operacoes reais!")
    print("Parametros otimizados para mercado atual:")
    print("   • Confiança minima: 55% (mais flexivel)")
    print("   • Volume: 0.01 lotes")
    print("   • R/R Ratio: 1:2")
    print("   • Stop dinamico baseado em ATR")
    print("="*60)
    
    # Confirmacao
    response = input("\nDeseja iniciar trading ao vivo otimizado? (SIM/NAO): ").upper().strip()
    if response != 'SIM':
    ...
    # Configurar sessao
    try:
        cycles = int(input("Numero de ciclos (padrao: 3): ") or "3")
        if cycles < 1 or cycles > 10:
            print("Ciclos devem ser entre 1 e 10")
            return
    except ValueError:
        print("Valor invalido, usando 3 ciclos")
        cycles = 3
    
    try:
        interval = int(input("Intervalo entre ciclos em segundos (padrao: 300): ") or "300")
        if interval < 60 or interval > 1800:
            print("Intervalo deve ser entre 60s e 1800s")
            return
    except ValueError:
        print("Valor invalido, usando 300s")
        interval = 300
